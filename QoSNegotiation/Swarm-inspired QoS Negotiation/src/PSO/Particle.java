...
public class Particle {
	...
	public void updatePBest(Particle gBest, float[] pBest) {
		...
		if (euclidianDistance(gBest, pBest) > euclidianDistance(gBest, this.distance))
	...
	public float euclidianDistance(Particle gBest, float[] curr) {
		...
		distance = (float) Math.pow(
				(curr[0] - gBest.getDistance()[0]), 2)
				+ (float) Math.pow((curr[1] - gBest.getDistance()[1]),
						2)
				+ (float) Math.pow((curr[2] - gBest.getDistance()[2]),
						2);
